...
def build_rag_context(query: str, n_results: int = 15) -> tuple[str, str, list[str]]:
    """Construit le prompt système, la query et la liste des sources.

    Utilisé par la Web UI (streaming SSE) et le serveur MCP.
    Retourne (system_prompt, user_query, sources).
    """
    from .retrieval import search_vault

    results = search_vault(query, n_results=n_results)
...
def _build_system_prompt(context_text: str) -> str:
    return f"""Tu es un assistant personnel intelligent connecté aux notes Obsidian de l'utilisateur.
